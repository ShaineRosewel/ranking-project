...
\begin{equation}
 P\left( \max_{k=1,2,\ldots,K} \left| \dfrac{\hat{\theta}_k-\theta_k}{\sigma_k} \right| \le t \right)  =1-\alpha.  \label{eq:prb}
\end{equation}
    ...
    \For {$b = 1, 2, \dots, B$}
        ...
        \State \begin{minipage}[t]{\dimexpr\linewidth-\algorithmicindent}
            Compute 
                \Statex \begin{minipage}{\linewidth}
                \centering
                $\hat\sigma^*_{b(k)} = \sqrt{\left[\text{kth ordered value among} \ \left\{ \hat{\theta}^{*2}_{b1} + \sigma_1^2, \hat{\theta}^{*2}_{b2} + \sigma_2^2, \dots, \hat{\theta}^{*2}_{bK} + \sigma_K^2 \right\}\right] - \hat {\theta}^{*2}_{b(k)}}$
                \end{minipage}
        \end{minipage}
        \State Compute 
...
\end{equation} \begin{equation}
  T_2 = \left[ \prod^K_{k=1} \Big | \Lambda_{Ok} \Big| \right]^{1/K}
  \label{eq:t2}
\end{equation}
...
For the simulation settings to be used in evaluating the performance of the proposed methodologies, we shall be varying the settings for the population mean $\boldsymbol{\theta}$, population variances $\sigma^2_1, \sigma^2_2,\ldots,\sigma^2_K$, population correlation matrix $\mathbf{R}$, and number of populations being ranked $K$. In carrying out the simulations, a nominal level of $1-\alpha=0.95$ will be used. Table \ref{tab:true-theta-table} summarizes the simulation settings.


```{r true-theta-table, tab.cap="True $\\boldsymbol{\\theta}$ values"}
...
dataset %>% 
  mutate(data_spread = factor(ifelse(data_spread == 'low', 'Low', ifelse(data_spread == 'med', 'Moderate', "High")),
...
column_names = c("K", "Variance",'$\\boldsymbol{\\theta}$', "$\\mathbf{R}$"),
main = TRUE
)
```


All correlation coefficients are set to be positive ($\rho > 0$) since it aligns better with the intended use case. The following configurations are used for $\mathbf{R}_{block}$:

```{r}
options(knitr.kable.NA = '')
source("/home/realiseshewon/PDev/kde-ranking/R/generate_true_theta.R")
df <- log_block_corrs(persist = FALSE)
prepare_block_corrs(df) %>% create_block_corr_table
```


```{r sigma-table, tab.cap="$\\sigma$ Values."}
sds<-readRDS("/home/realiseshewon/PDev/kde-ranking/data/mean_travel_time_ranking_2011.rds")


ks <-seq(10,50,10)
sd_setup <- data.frame(K=ks, sigma = sapply(ks, function(x) paste(sds$S[1:x], collapse = ", ")))


create_table_for_true_theta(sd_setup,
                            column_names = c("K", '$\\sigma$'),
                            main = FALSE)
```
