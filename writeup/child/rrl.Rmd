...
@jelle applied Tukey's honest significant difference (HSD) test all $H_{0;k,k'}: \theta_k \neq \theta_{k'}$ for all $k \neq k' \in {1, ..., K}$ at level $\alpha$ to come up with a joint confidence set for ranks expressed as

\begin{equation}
  \left(
  1 + \#\left\{ j: \frac{\hat{\theta}_k - \hat{\theta}_{k'}}{\sqrt{\sigma^2_k + \sigma^2_{k'}} }> q_{1-\alpha} \right\},
  \;
  K - \#\left\{ j: \frac{\hat{\theta}_k - \hat{\theta}_{k'}}{\sqrt{\sigma^2_k + \sigma^2_{k'}} } < -q_{1-\alpha} \right\}
  \right), \notag
\end{equation}
where $q_{1-\alpha}$ is the $(1-\alpha)$ quantile of of the distribution of the studentized range, 
\begin{equation}
\underset{k,k'=1, \dots n}{\max} \frac{\lvert \theta_k-\theta_{k'}\rvert}{\sqrt{\sigma^2_k + \sigma^2_{k'}}} \notag
\end{equation}
They showed this to yield uniformly narrower intervals than that of Klein's, for the case when $\sigma$s are equal. This has a simultaneous coverage of at least $1-\alpha$ and exactly $1-\alpha$ when all true performances are equal. However, their approach tends to be overly conservative, showing coverage levels between 0.996 and 1.0 at a 0.90 nominal level in simulations, when performances differ (i.e., there are no ties). They also demonstrated that as the true performance differences increase from 0 to 0.5, the coverage quickly increases from the nominal level to 1. As a remedy, they proposed a rescaling technique that brings coverage closer to the nominal level, though it remains conservative (e.g., from 1.0 to 0.978, from 0.998 to 0.961---at 0.90 confidence level).

@mogstadt23 closely resembles the procedure by Klein since they also constructed the confidence region for ranks basing from the simultaneous confidence region of estimates. However, they defined ranks in the opposite way (descending rather that ascending order) and constructed the rectangular confidence region in \ref{eq:mogstadt23_ci}, from the pairwise differences of estimators $\hat{\theta}_1, \dots, \hat{\theta}_K$ and an estimator of the variance of $\hat{\theta}_{k}-\hat{\theta}_{k'}$, $\widehat{SE}^2_{kk'}$. $P_k$ is the distribution from which $\hat{\theta}_k$ is estimated; $\hat{P}_k$ denotes the estimate of $P_k$.

\begin{equation}
\begin{split}
C(1-\alpha, S) &= \prod_{(k,k') \in S} \left[ 
\hat{\theta_k} - \hat{\theta}_{k'} \pm \widehat{SE}^2_{kk'} \;L^{-1}(1-\alpha, S, \hat{P})
\right]
, \\
&\qquad\qquad\qquad\ S \subseteq \{(k,k') \in K\times K: k\neq k'\}
\end{split}
...
\begin{equation}
  L(x, S, P) = P\left\{ \underset{(k,k')\in S}{\max}
  \frac{\lvert
  \hat{\theta}_k - \hat{\theta}_{k'} -\Delta_{k,k'}(P)
