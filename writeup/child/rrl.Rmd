...
For the choice of $c_k$, Dunn advised to assume independence with $c_1 = \dots = c_K = c_\gamma$ where $\gamma$ is the individual confidence level so that
$$
\prod^K_{k=1} P\left( \lvert X_k \rvert \leq c_k\right) = 
\prod^K_{k=1} \left( 1-\gamma \right) = (1-\gamma)^K = 1-\alpha
$$
and deriving $\gamma$ returns $1-{(1-\alpha)}^{1/k}$. Under this condition, a two-sided $100(1-\gamma) \%$ confidence interval for the parameter $\theta_k = \mu_k$ is given by
