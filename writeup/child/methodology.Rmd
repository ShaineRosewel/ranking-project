This section introduces the proposed methodologies to obtain confidence regions for the unknown overall true ranking. It extends approaches from @klein: the Bonferroni correction and the independence assumption. The case when items ranked are assumed to have nonzero correlation is addressed. Approaches introduced are based on parametric bootstrap. Sections \@ref(sec:confregionalgo) discusses the algorithms employed to compute for the joint confidence regions. This includes a non-rank and rank-based methods. Calculated joint confidence regions are then assessed on the basis of coverage and metrics that measure the tightness of estimated confidence regions. These are tackled in section \@ref(sec:evaluation).
...
The proposed approaches are constructed to account for assumed correlation among items being ranked. For this, various correlation structures $\boldsymbol{\rho}$ are examined (Refer to section \ref{sec:corrstructures}) and used in the calculation of the variance covariance matrix (See Equation \ref{eq:sigma_matrix}.).
...
where $\boldsymbol{\Delta} = \text{diag} \left\{ \sigma^2_1, \sigma^2_2, \dots, \sigma^2_K \right\}$, with known $\sigma_k$'s and $\boldsymbol{\rho}$. This form of $\boldsymbol{\Sigma}$ is will be used in sections \@ref(sec:nonrankbased) and \@ref(sec:rankbased)
