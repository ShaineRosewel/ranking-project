...
Once the confidence intervals in (\ref{eq:rev1}) have been obtained, we can then use the result of @klein in (\ref{eq:joint_cov2}) to get the lower and upper bounds on the ranks $r_k, k=1,2,\ldots,K$. That is, we also get ajoint confidence region for $r_1, r_2, . . . , r_K$.
...
sig5 <- paste0("(",paste(df$S[1:50], collapse = ", "),")")
sig5
```
...
\hline
\hline
\multicolumn{2}{c}{\textbf{K = 5}} \\
\hline
\multirow{3}{11cm}{(22.5, 24.9, 27.5, 24.3, 24.8)$'$} 
& $\mathbf{R}_{\text{eq}}, \rho = 0.1, 0.5, 0.9$ \\
& $\mathbf{R}_{\text{block}}$ \\
\hline
\multirow{3}{11cm}{(21.5, 25.7, 30.5, 24.7, 25.5)$'$} 
& $\mathbf{R}_{\text{eq}}, \rho = 0.1, 0.5, 0.9$ \\
& $\mathbf{R}_{\text{block}}$ \\
\hline
\multirow{3}{11cm}{(19.9, 27, 34.9, 25.3, 26.7)$'$} 
& $\mathbf{R}_{\text{eq}}, \rho = 0.1, 0.5, 0.9$ \\
& $\mathbf{R}_{\text{block}}$ \\
\hline
...
\hline
$K$ & $\mathbf{R}_{\text{block,K}}$ \\
\hline
\hline
5 & \\
\hline
10 & $\mathbf{R}_{\text{eq},n=3}$ with $\rho = 0.1$, $\mathbf{R}_{\text{eq},n=3}$ with $\rho = 0.5$, $\mathbf{R}_{\text{eq},n=4}$ with $\rho = 0.9$ with $\rho = 0.1$ in the off-diagonal blocks \\
\hline
20 &  \\
\hline
30 &  \\
...
Each $k$ shares the same population variances $\sigma^2_1,\sigma^2_2,\ldots,\sigma^2_K$:
...
```{r simulation-table, results='asis', echo=FALSE, eval = FALSE}
library(dplyr)
library(tidyr)

K <- c(5, 10)
theta <-c(2.3,6,9)# my_list
R_type <- c("R_eq", "R_block", "R_matern")

grid <- expand.grid(K = K, theta = theta, R = R_type) %>%
  arrange(K, theta, R)

grid <- grid %>%
  mutate(R_desc = case_when(
    R == "R_eq" ~ "$\\mathbf{R}_{\\text{eq}}, \\rho = 0.1, 0.5, 0.9$",
    R == "R_block" ~ "$\\mathbf{R}_{\\text{block}}$",
    R == "R_matern" ~ "$\\mathbf{R}_{\\text{matern}}$ (using distances between states)"
  ))
...
grid <- grid %>%
  ...
  ungroup() %>%
  group_by(K) %>%
  mutate(K_rows = sum(theta_rows)) %>%
  ungroup()

# Identify when K and theta change
grid <- grid %>%
  group_by(K) %>%
  mutate(
    K_cell = ifelse(row_number() == 1,
                    paste0("\\multirow[t]{", K_rows, "}{*}{", K, "}"), "")
  ) %>%
  group_by(K, theta) %>%
  mutate(
    theta_cell = ifelse(row_number() == 1,
                        paste0("\\multirow[t]{", theta_rows, "}{*}{", theta, "}"), "")
  ) %>%
  ungroup()
...
grid <- grid %>%
  group_by(K) %>%
  mutate(
    add_hline = ifelse(row_number() == n(), "\\hline", ""),
    add_cline = ifelse(row_number() %% 3 == 0 & row_number() != n(), "\\cline{2-3}", "")
  ) %>%
  ungroup()
...
table_lines <- grid %>%
  mutate(
    line = paste0(K_cell, " & ", theta_cell, " & ", R_desc, " \\\\ ", add_cline, add_hline)
  ) %>%
  pull(line)

# Wrap into LaTeX table
cat("\\begin{table}[H]",
    "\\centering",
    "{\\fontsize{10pt}{12pt}\\selectfont",
    "\\begin{tabular}{c l l}",
    "\\hline",
    "$K$ & $\\boldsymbol{\\theta}$ & $\\mathbf{R}$ \\\\",
    "\\hline",
