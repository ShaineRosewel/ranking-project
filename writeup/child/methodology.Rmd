...
out %>%
  ...
  kable("latex", booktabs = TRUE, longtable = FALSE,
        col.names = c("K", "sd", "Parametric", "Non-rankbased", "Independent", "Bonferroni"),
        caption = "Simulation results for coverage probabilities when correlation is zero.") %>%
...
As the correlation increases, the coverage of both independent and Bonferroni CIs exceed the nominal value while that of nonrank-based method remains close to it. This holds regardless of $K$ and $sd$. See Table \@ref(tab:nonzerocorrel). This suggests that when items being ranked are correlated, the proposed method is preferable, as it maintains coverage close to the nominal level.
...
out %>%
  ...
  arrange(corr, K) %>%
  kable("latex", booktabs = TRUE, longtable = FALSE,
        col.names = c("corr","K", "Non-rankbased", "Independent", "Bonferroni"),
        caption = "Simulation results for coverage probabilities when correlation is nonzero.") %>%
  kable_styling(latex_options = c("repeat_header")) %>%
  collapse_rows(columns = 1, latex_hline = "major") %>%   # <-- merges K column
...
names(df_wide) <- sub("_.*", "", names(df_wide))

# build table
df_wide %>%
  kable("latex", booktabs = TRUE, longtable = FALSE,
        caption = "Simulation results for $T_1$ probabilities when correlation is nonzero.") %>%
  kable_styling(latex_options = "repeat_header") %>%
