...
>    2. obtain joint confidence intervals for the $r_1,r_2,\ldots,r_K$ using a result from @klein). 
...
If the condition in (\ref{eq:theta_int}) holds, the main result from @klein gives a range for the value of \(r_k\) for each \(k \in \left\{1, 2, \dots, K\right\}\) as follows:
...
## Measurement across geographies --- @klein
