...
\item
  ...
  relative standings between the units.
\item
  Because rankings based on
  \(\hat\theta_1,\hat\theta_2,\ldots, \hat\theta_K\) can vary due to
...
\tightlist
\item
  Mohamad et al.~(2019) apply Tukey's Honest Significant Difference
  (HSD) to test \(H_0: \theta_j - \theta_{k} = 0\) for all
  \(j \neq k \in {1, ..., K}\) at level \(\alpha\).
\item
  Mogstad et al.~(2024) construct the rectangular confidence region from
  the pairwise differences of the estimators
  \(\hat{\theta}_1, \dots, \hat{\theta}_K\) and an estimator of the
  variance of \(\hat{\theta}_{j}-\hat{\theta}_{k}\).
\end{itemize}
...
\begin{frame}{Other related studies}
\phantomsection\label{other-related-studies-1}
\begin{itemize}[<+->]
\tightlist
\item
  Goldstein \& Spiegelhalter (1996) use multilevel models, in the
  context of ranking education and health institutions (e.g., schools,
  hospitals, medical practitioners, etc.), to address the hierarchical
  nature of data structures associated with institutional performance.
\item
  Hall \& Miller (2009) suggest using an ``independent component''
  version of the bootstrap on the sample, where m-out-of-n bootstrap (m
  \textless{} n) is applied as though the ranked variables were
  statistically independent.
\item
  Zhang et al.~(2013) analyze U.S. age-adjusted cancer incidence and
  mortality rates across states and counties by computing individual and
  overall simultaneous confidence intervals for age-adjusted health
  index using the Monte Carlo method.
\item
  Bazylik et al.~(2025), in their recent study, tackle the ranking of
  political candidates or parties using the estimated share of support
  each one receives in surveys. They address the dependence attributed
  to the success probabilities of different categories by using their
  proposed bootstrap algorithm.
\end{itemize}
...
population variances \(\sigma^2_1, \sigma^2_2,\ldots,\sigma^2_K\). A
nominal level of \(1-\alpha=0.95\) will be used.
