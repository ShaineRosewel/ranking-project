...
dataset %>% select(c(k, theta_k, variance, rhat_k)) %>% arrange(rhat_k)
```
...
generate_synthetic_dataset <- function(K = 51, seed = 42) {
  ...
  theta_k <- seq(1, 0.05*K, length.out = K)
  # fixed small standard errors
  #S <- rep(0.9, K)
  
  template <- c(rep(0.05, ceiling(2*K / 3)), rep(1.2, floor(K / 3)))
  S <- rep(template, length.out = K)
...
get_coverage <- function(dataset_or, 
  ...
  dataset$theta_k <- NULL # remove theta k from the dataset
  dataset$theta_k <- t(theta_hat) # then replace it with the sampled so usable in algo 1 fcn
...
### How often is a state missed across reps? {.tabset}
...
results <- lapply(Ks, function(K) {
  ...
  data.frame(
    ...
    Within_CI_mean = mean(a$inside_check_1),
    Within_CI_max = max(a$inside_check_1),
    Within_CI_min = min(a$inside_check_1),
    T_mean = mean(a$t2)
  )
})
...
### Results for different alpha, **100** reps
