...
misses %>%
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(color = "On Target?") + xlab("States with all S = 0.2") + ylab("Difference from closest bound") + scale_size_continuous(trans = "sqrt", range = c(0, 3)) + guides(size = FALSE)
```

K = 30

```{r echo = FALSE, results = "asis", fig.width=10}
misses <- get_iter_misses(K = 30)
misses$diff1 <- misses$theta - misses$kde_ci_lower
misses$diff2 <- misses$theta - misses$kde_ci_upper
