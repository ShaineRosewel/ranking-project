...
set.seed(3)
for (i in 1:K) {
  print(rnorm(1, dataset[i,'theta_k'], sqrt(dataset[i,'variance'])))
}
```
...
```{r}
r <- mat1_sorted - matrix(t(dataset['theta_k']),B, K, byrow=TRUE)
```

4. 

Kernel density for each i (for each k)
...
estimate_CDF <- function(i){
  S = sd(r[,i])
  IQR = quantile(r[,i], .75) - quantile(r[,i], .25)
  h_i <- 0.9*min(S, IQR*B^(1/5))
  return(kde_cdf_function(r[,i], h_i))
}

estimated_CDF <- lapply(1:K, estimate_CDF)
```
...
dataset['lower'] <- dataset['theta_k'] - dataset['F.inv_u'] 
dataset['upper'] <- dataset['theta_k'] - dataset['F.inv_1-u'] 
