...
dataset %>% select(c(k, theta_k, variance, rhat_k, S, moe_k)) %>% arrange(rhat_k)
```
...
```{r fig.height=3, fig.align='center', fig.width=20, echo = FALSE}
library(tidyr)
library(ggplot2)

dat10 <- data.frame(thetahat_star)
x <- paste0("thetahat_star", sprintf("%02d", 1:K))
colnames(dat10) <- x
...
```{r fig.height=4, fig.align='center', fig.width=20, echo = FALSE}
...
x <- paste0("rstar", sprintf("%02d", 1:K))
colnames(dat10) <- x
...
x <- paste0("Ystar", sprintf("%02d", 1:K))
colnames(dat10) <- x
...
length(Ustar[Ustar<= uhat])/length(Ustar)
```
...
#dataset_or <- generate_synthetic_dataset(K = K, seed = 42) #TRUE
#dataset<-dataset_or
theta_hat <- foreach(i = 1:K, 
                     .combine = cbind) %do% {
                       rnorm(1, 
...
dataset$theta_k <- NULL # remove theta k from the dataset
...
```{r, echo = FALSE}
kable(output[[10]], row.names = FALSE) %>% kable_styling(bootstrap_options = "responsive")
```

##### K = 20

```{r, echo = FALSE}
kable(output[[20]], row.names = FALSE) %>% kable_styling(bootstrap_options = "responsive")
```

##### K = 30

```{r, echo = FALSE}
kable(output[[30]], row.names = FALSE) %>% kable_styling(bootstrap_options = "responsive")
```

##### K = 40

```{r, echo = FALSE}
kable(output[[40]], row.names = FALSE) %>% kable_styling(bootstrap_options = "responsive")
```

##### K = 51

```{r, echo = FALSE}
kable(output[[51]], row.names = FALSE) %>% kable_styling(bootstrap_options = "responsive")
```
...
K <- 10
df <- readRDS("../data/mean_travel_time_ranking_2011.rds")
dataset_or <- df[order(df$S),][1:K,]


kable(dataset_or) %>% kable_styling(bootstrap_options = "responsive")
```
...
K <- 10
df <- readRDS("../data/mean_travel_time_ranking_2011.rds")
dataset_or <- df[order(df$k),][1:K,] ### CONTAINS YOUR TRUE THETA OR YOUR MEANS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
get_coverage <- function(dataset_or, 
  ...
  foreach(iter = 1:reps, 
          ...
          .export = c("run_algorithm1", "estimate_CDF","get_ranks", 
                      "get_t2", "kde_cdf_function",'get_inner_max',"inverse",
  ...
  inside_check_1 <- sum((sorted_thetahat - result$Finv_u < sorted_theta) &
                        (sorted_theta < sorted_thetahat - result$Finv_1_u))

  tab_result <- result$interval_table
  inside_check_2 <- sum((sorted_theta > tab_result$kde_ci_lower) &
                        (sorted_theta < tab_result$kde_ci_upper))
  ...
  inside_check_3 <-  sum(((1-uhat) < Ystar) & ((Ystar) < uhat))
  ...
  data.frame(t2 = t2_val,
             inside_check_1 = inside_check_1,
             inside_check_2 = inside_check_2,
             inside_check_3 = inside_check_3)
}
}


#   #sorted_indices <- result$sorted_indices #sorted_theta <-
#  # sort_reference$theta[sorted_indices]
# 
#   sorted_theta <- sort(dataset_or$theta_k) # TRUE VALUE #sorted_thetahat <-
#   #sort_reference$theta_hat[sorted_indices]
# 
#   #inside_check_1 <- sum((sorted_thetahat - result$Finv_u < sorted_theta) & #
#   #(sorted_theta < sorted_thetahat - result$Finv_1_u))
# 
#   tab_result <- result#$interval_table
# 
#   inside_check_2 <- all((sorted_theta >= tab_result$kde_ci_lower) & (sorted_theta <= tab_result$kde_ci_upper))
# 
# 
#   indicator <- all(result$kde_ci_lower <= sorted_theta) & all(sorted_theta <=result$kde_ci_upper)
# 
#   #Fhat <- result$Fhat #uhat <- result$uhat #rstar <- sorted_thetahat -
#  # sorted_theta
# 
#   #Ystar <- sapply(1:length(Fhat), function(i) {
#   #  Fhat[[i]](rstar[i])
...
#   data.frame(t2 = t2_val,
#              #inside_check_1 = inside_check_1,
#              original =inside_check_2,
#              inside_check_2 = indicator)
#              #inside_check_3 = inside_check_3)
# }
# }
```
...
K=51
set.seed(4)
#df <- generate_synthetic_dataset(K = K, seed = 42)
...
for (K in c(10, 20, 30, 40, 51)) {
  ...
  saveRDS(coverage_output_df,  paste0("../output/test/coverage_probability_",K, ".rds"))
  coverage_output_df
...
for (alpha in c(0.01, 0.05, 0.10)) {
  for (K in c(51, 40, 30, 20, 10)) {
    
    set.seed(4)
    dataset_or <- df[order(df$k), ][1:K, ]
    
    tic(paste0("Running... alpha=", alpha, ", K=", K))
    coverage_output_df <- get_coverage(dataset_or, 
                                       K, 
                                       reps = 100, 
                                       B = 1000, 
                                       alpha = alpha)
    toc()
    coverage_output_df <- coverage_output_df %>%
      mutate(K = K, alpha = alpha)
    results_list[[paste(K, alpha, sep = "_")]] <- coverage_output_df
    

    saveRDS(
      coverage_output_df,
      paste0("../output/test/coverage_probability_K", K, "_alpha", alpha, ".rds")
    )
  }
}
...
saveRDS(final_results, "../output/test/coverage_probability_all.rds")
```




### How often is a state missed across reps?  to redo {.hidden .unlisted .unnumbered}


Aim is to observe which states usually fail to make it within the estimated interval, making the coverage low, even if it is just one miss. It appears that states with higher SD usually fail to make it within the interval. Plots below show misses as triangular points. They are below the y = 0 line if they are closer to the lower bound and above it if closer to upper bound. 

#### K = 10

```{r echo = FALSE, results = "asis", fig.width=9, fig.height=5}
library(arrow)
library(ggplot2)
source('../R/algo2_helper.R')
K = 10
file_list <- list.files(path = paste0("../output/iters/test/K",K), pattern = "\\.feather$", full.names = TRUE)
# df <- readRDS("../data/mean_travel_time_ranking_2011.rds")


plot_misses(prepare_misses(file_input_path = paste0("../output/iters/test/K", K), K = 10))
```
...
```{r echo = FALSE, results = "asis", fig.width=9, fig.height=5}
plot_misses(prepare_misses(file_input_path = paste0("../output/iters/K", 20), K = 20))
```

#### K = 30

```{r echo = FALSE, results = "asis", fig.width=9, fig.height=5}
plot_misses(prepare_misses(file_input_path = paste0("../output/iters/K", 30), K = 30))
```

#### K = 40

```{r echo = FALSE, results = "asis", fig.width=9, fig.height=5}
plot_misses(prepare_misses(file_input_path = paste0("../output/iters/K", 40), K = 40))
```

#### K = 51

```{r echo = FALSE, results = "asis", fig.width=9, fig.height=5}
plot_misses(prepare_misses(file_input_path = paste0("../output/iters/K", 51), K = 51))
```
...
results <- lapply(Ks, function(K) {
  a <- readRDS(paste0("../output/test/coverage_probability_", K, ".rds"))
  #a <- readRDS(paste0("../output/coverage_probability_", K, ".rds"))
  #print(a)
  data.frame(
    K = K,
    Coverage = mean(a$inside_check_1 == K),  # only TRUE if inside == K
...
### Results for different alpha, **100** reps - to redo {.hidden .unlisted .unnumbered}

```{r}
library(dplyr)
final_results <- readRDS("../output/test/coverage_probability_all.rds")
prepped <- final_results %>% 
  mutate(ft = K==inside_check_1) %>% 
  select(alpha, K, ft, t2) %>% 
  group_by(alpha, K) %>% 
  summarise(cov = mean(ft), mean_t2 = mean(t2)) %>% 
  arrange(K)


kable(prepped) %>% kable_styling(bootstrap_options = "responsive")
```
...
results <- lapply(Ks, function(K) {
  # "../output/coverage_probability_"
  fname <- "../output/test/coverage_probability_"
  a <- readRDS(paste0(fname, K, ".rds"))
  data.frame(K = K, inside = a$inside_check_1)
})
...
### Experimental runs results {.hidden .unlisted .unnumbered}

- K = 10, 100 reps, cov: changing rstar from `rstar <- sorted_thetahat_star - matrix(sorted_theta_hat,B, K, byrow=TRUE)` to 
`rstar <- sweep(sorted_thetahat_star, 2, colMeans(sorted_thetahat_star), FUN = "-")` increased coverage by ~0.10  
- K = 51, 100 reps, cov: changing rstar from `rstar <- sorted_thetahat_star - matrix(sorted_theta_hat,B, K, byrow=TRUE)` to 
`rstar <- sweep(sorted_thetahat_star, 2, colMeans(sorted_thetahat_star), FUN = "-")` increased coverage by ~0.06
- simple dgp via `generate_synthetic_dataset` produced close to expected result
- Using ecdf instead of pnorm did not change the coverage for K=10

### Experimental runs results - to redo {.hidden .unlisted .unnumbered}

#### alpha {.hidden .unlisted .unnumbered}
...
## Looped coverage {.hidden .unlisted .unnumbered}
