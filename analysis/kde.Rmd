...
get_coverage <- function(dataset_or, K, reps = 100, B=1000, alpha= 0.10){
  ...
  print(sprintf("../output/iters/K%d/sorted_ci_%04d.feather", K, iter))
...
dataset_or <- df[order(df$k),][1:K,]
misses <- get_iter_misses(K=1, file_list=file_list)
...
misses <- get_iter_misses(K=20, file_list=file_list)
misses$diff1 <- misses$theta - misses$kde_ci_lower
misses$diff2 <- misses$theta - misses$kde_ci_upper
...
misses <- get_iter_misses(K, file_list=file_list)
