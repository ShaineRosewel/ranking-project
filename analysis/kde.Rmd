...
get_coverage <- function(dataset_or, K, reps = 5, B=1000, alpha= 0.10){
  foreach(iter = 1:reps, .combine = rbind,.packages = c("foreach"), 
          .export = c("run_algorithm1", 'calculate_variance', "estimate_CDF", 
  ...
  inside_check <- sum(sorted_ci$otheta_k >= sorted_ci$kde_ci_lower & sorted_ci$otheta_k <= sorted_ci$kde_ci_upper)
...
tic("Running...")
coverage_output_df <- get_coverage(dataset_or, K, reps = 5, B=1000, alpha=0.1)
toc()
stopCluster(cl)
...
```{r}
# t
mean(coverage_output_df$t2)
```


```{r}
# proportion 
mean(coverage_output_df$inside==K)
```
...
```{r}
a251<-readRDS("../output/coverage_probability_51.rds")
print("K = 51")
print(paste("Coverage:", mean(a251$inside)))
print(paste("T:",mean(a251$t2)))
cat("\n")
a251<-readRDS("../output/coverage_probability_30.rds")
print("K = 30")
print(paste("Coverage:", mean(30==a251$inside)))
print(paste("T:",mean(a251$t2)))
print(paste("Average within CI:",mean(a251$inside)))
cat("\n")
a251<-readRDS("../output/coverage_probability_10.rds")
print("K = 10")
print(paste("Coverage:", mean(10==a251$inside)))
print(paste("T:",mean(a251$t2)))
print(paste("Average within CI:",mean(a251$inside)))
```
