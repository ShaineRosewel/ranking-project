...
knitr::opts_chunk$set(echo = TRUE,fig.align = "center", fig.width = 5, fig.height = 3, comment="")
```
...
dataset['lower_joint'] <- dataset['theta_k'] - dataset['F.inv_u'] 
dataset['upper_joint'] <- dataset['theta_k'] - dataset['F.inv_1-u'] 
...
run_algorithm1 <- function(B, dataset) {
  dataset['variance'] <- lapply(dataset['moe_k'], calculate_variance)
  K <- dim(dataset)[1]
  set.seed(4)
  mat1 <- foreach(i = 1:K, .combine = cbind) %do% {
  ...
  dataset['lower_joint'] <- dataset['theta_k'] - dataset['F.inv_u'] 
  dataset['upper_joint'] <- dataset['theta_k'] - dataset['F.inv_1-u']
  return(dataset[, c('rhat_k', 'k', 'theta_k', 'lower_joint', 'upper_joint')])
}
```
...
for (K in c(10, 20, 30, 40, 51)) {
  ...
  filename <- paste0("../data/joint_intervals_", K, ".rds")
...
for (K in c(10, 20, 30, 40, 51)) {
  ...
  tuple_list <- t(apply(output[[K]][,c('lower_joint', 'upper_joint')], 1, function(row) as.numeric(row)))
  
  output[[K]]['rank_range'] <- sapply(1:K, function(x) get_ranks(x, tuple_list))
  print(paste("**** Output shown for K=", K, "****"))
  
  output[[K]]['sample_rank'] <- rank(output[[K]]$theta_k, ties.method = "min")
