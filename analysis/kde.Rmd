...
```{r}
kable(head(df[,c("k", "bonf_rank_lower", "bonf_rank_upper", "bonf_ci_lower", "bonf_ci_upper", 'bonf_Lambda_Ok_length', 'bonf_rank_range_length')])) %>% kable_styling(bootstrap_options = "responsive")
```
...
```{r}
kable(head(df[,c("k", "ind_rank_lower", "ind_rank_upper", "ind_ci_lower", "ind_ci_upper", 'ind_Lambda_Ok_length', 'ind_rank_range_length')])) %>% kable_styling(bootstrap_options = "responsive")
```
...
get_coverage <- function(dataset_or, K, reps = 100, B=1000, alpha= 0.10){
  foreach(iter = 1:reps, .combine = rbind,.packages = c("foreach", "arrow"),
  ...
  sort_reference <- data.frame(theta = dataset$theta_k, theta_k = t(mat2))
  ...
  ci <- merge(result, sort_reference, by = "theta_k", all = FALSE)
  sorted_ci <- ci[order(ci$theta),]
  
  write_feather(sorted_ci, sprintf("../output/iters/K%d/sorted_ci_%04d.feather", K, iter))

  inside_check <- sum(sorted_ci$theta >= sorted_ci$kde_ci_lower & sorted_ci$theta <= sorted_ci$kde_ci_upper)
  t2_val <- get_t2(result$ind_rank_range_length)
  
  data.frame(t2 = t2_val, inside = inside_check)
  #sorted_ci$iteration <- rep(iter, K)#
  #sorted_ci
}
}
tic("Running...")
...
#8999.084 sec elapsed 51
#5701.035 sec elapsed 30
#1885.258 sec elapsed 10
```
...
library(arrow)
file_list <- list.files(path = "../output/iters/K10", pattern = "\\.feather$", full.names = TRUE)

coverage_output_df <- lapply(file_list, function(file) {
  df <- read_feather(file)
  df$filename <- basename(file)  # or use tools::file_path_sans_ext(basename(file)) to remove .feather
  return(df)
}) %>%
  bind_rows()
...
kable(subs %>% select(c('k', "theta_k", "theta", "kde_ci_lower", "kde_ci_upper", "theta_within", "filename")) %>% filter(!theta_within)) %>% kable_styling(bootstrap_options = "responsive")
```

K = 20 

```{r echo = FALSE, results = "asis"}
file_list <- list.files(path = "../output/iters/K20", pattern = "\\.feather$", full.names = TRUE)
...
```{r echo = FALSE, results = "asis"}
file_list <- list.files(path = "../output/iters/K30", pattern = "\\.feather$", full.names = TRUE)
...
```{r echo = FALSE, results = "asis"}
file_list <- list.files(path = "../output/iters/K40", pattern = "\\.feather$", full.names = TRUE)
...
```{r echo = FALSE, results = "asis"}
file_list <- list.files(path = "../output/iters/K51", pattern = "\\.feather$", full.names = TRUE)
