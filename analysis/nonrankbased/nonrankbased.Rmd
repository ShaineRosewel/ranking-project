...
get_coverage <- function(dataset_or, 
  ...
  gamma = 1-(1-alpha)^(1/K)
  z1 = qnorm(gamma/2)
  ind_lower_ci <- means - z1*dataset$S
  ind_upper_ci <- means + z1*dataset$S
  
  inside_check_ind <- sum((true_theta >= ind_lower_ci) &
                        (true_theta <= ind_upper_ci))
  
  
  z2 = qnorm((alpha/2)/2)
  bonf_lower_ci <- means - z2*dataset$S
  bonf_upper_ci <- means + z2*dataset$S

  inside_check_bonf <- sum((true_theta >= bonf_lower_ci) &
                        (true_theta <= bonf_upper_ci))
...
kable(results %>% arrange(-sd) %>% dplyr::select(K, sd, Coverage, T_mean)) %>% kable_styling(bootstrap_options = "responsive")
```
