...
get_coverage <- function(dataset_or, 
  ...
  means <- dataset$theta_k # true theta!
  ...
  dataset$theta_k <- theta_hat # theta hat
...
kable(results %>% arrange(-sd) %>% dplyr::select(K, sd, corr,Coverage_nonrank,Coverage_ind, Coverage_bonf, T_mean_nonrank , T_mean_ind, T_mean_bonf)) %>% kable_styling(bootstrap_options = "responsive")
```
